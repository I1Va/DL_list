...
static void list_make_free_connections(DL_list_t *list, const size_t left_idx) { // static
    for (size_t i = left_idx; i < list->size; i++) {
...
bool DL_list_ctor(DL_list_t *list, const size_t size) {
    ...
    list_make_free_connections(list, 1);
...
bool resize_up(DL_list_t *list) {
    ...
    DL_list_node_t *prev_data_ptr = list->data;
    list->size *= resize_up_coeff;

    void *new_data_ptr = realloc(list->data, list->size * sizeof(DL_list_node_t));
    if (new_data_ptr == NULL) {
        DEBUG_DL_LIST_ERROR(DL_ERR_REALLOC, "")
        return false;
    }
    list->data = (DL_list_node_t *) new_data_ptr;

    for (size_t i = 0; i < old_size; i++) {
        if (list->data[i].next != NULL) {
            list->data[i].next += (list->data - prev_data_ptr);
        }
        if (list->data[i].prev != NULL) {
            list->data[i].prev += (list->data - prev_data_ptr);
        }
    }

    list->free_node += (list->data - prev_data_ptr);
    list->free_node->next = &list->data[old_size];
    list->data[old_size - 1].next = &list->data[old_size];

    list_make_free_connections(list, old_size);
